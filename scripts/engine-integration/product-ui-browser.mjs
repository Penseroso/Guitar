...
try {
    ...
    await send('Page.addScriptToEvaluateOnNewDocument', { source: `window.engineSmoke={starts:0,stops:0,maxBytes:0,maxCards:0,viewRequests:0,kinds:[]};
        window.Worker=new Proxy(window.Worker,{construct(Target,args){const w=new Target(...args);window.engineSmoke.starts++;
            ...
            w.addEventListener('message',e=>{if(e.data?.protocol!=='engine-worker-v2')return;
    ...
    measurements.push({ scenario: 'Chord entry to first rendered exact page, release server', elapsedMs: performance.now() - started });
    check(await evaluate(`${cards}.length===6`), 'Initial exact page renders six cards');
    const recommendedCount=Number(await evaluate(count));
    await choice('Voicing surface','all');
    check(Number(await evaluate(count))>recommendedCount,'All exposes the complete survivor view after the selective Recommended default');
    ...
    check(await evaluate(`${section}.innerText.includes('outside this surface or these filters')`), 'Off-filter selection is explained');
    ...
    await screenshot('product-ui-mobile-emulation');
    await size(1440, 1000); await screenshot('product-ui-desktop');
    ...
    const evidence = { schema: 'engine-ui-browser-v1', measuredAt: new Date().toISOString(), browser: version.Browser, url: base,
        build: 'Next production server; functional UI regression only', passed: true, checks, measurements, instrumentation, errors,
        screenshots: ['docs/implementation/screenshots/product-ui-mobile-emulation.png', 'docs/implementation/screenshots/product-ui-desktop.png'],
        limits: ['Desktop Chrome only; mobile viewport/touch emulation is not real mobile-device evidence.', 'Functional release build checks; not a repeated latency or complete assistive-technology audit.', 'No human preference, playability or comfort benefit claim.'] };
    await mkdir('docs/implementation', { recursive: true }); await writeFile('docs/implementation/product-ui-browser.json', `${JSON.stringify(evidence, null, 2)}\n`);
