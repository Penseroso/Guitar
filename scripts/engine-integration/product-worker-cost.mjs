...
for(const path of ['src/domain/chord/engine/packedStore.ts','src/domain/chord/engine/workerService.ts','src/domain/chord/engine/workerProtocol.ts','src/domain/chord/engine/session.ts','src/domain/chord/engine/physicalDemand.ts','src/domain/chord/engine/practicalVocabulary.ts','src/domain/chord/engine/practicalVocabularyData.json','src/domain/chord/engine/recommendedSurface.ts'])evidence.sourceHashes[path]=createHash('sha256').update(await readFile(path)).digest('hex');
try{
    ...
    await writeFile('docs/implementation/product-worker-cost.json',JSON.stringify(evidence,null,2)+'\n');
...
function setupHarness(){
    ...
    function request(kind,payload,terminal){
        ...
        return new Promise((resolve,reject)=>{const timer=setTimeout(()=>{pending=null;reject(Error('Worker operation timed out'));},20000);pending={operationId,started,terminal,resolve:value=>{clearTimeout(timer);resolve(value);},reject:error=>{clearTimeout(timer);reject(error);}};
            worker.postMessage({protocol:'engine-worker-v2',sessionId:'cost',requestRevision:revision,viewRevision,operationId,kind,payload});});
    }
    ...
    async function query(chordId,context,maxModeledFret=15){revision++;viewRevision=1;return request('START',{view:{schema:'surface-request-v2',surface:'recommended',view:{}},intent:{schema:'intent-v1',chordId,rootPitchClass:0,context,instrument:{kind:'six-single-strings-12edo',tuningMidi:[64,59,55,50,45,40],maxModeledFret}},pageSize:30},['EXACT_PAGE','PAUSED']);}
