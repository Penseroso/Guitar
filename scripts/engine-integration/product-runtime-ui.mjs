...
let priorEvidence;
try { priorEvidence = JSON.parse(await readFile('docs/implementation/product-release-runtime-ui.json', 'utf8')); } catch { /* First evidence run. */ }
const evidence = { schema: 'engine-release-runtime-ui-v1', measuredAt: new Date().toISOString(), url,
    ...
    scope: 'Actual release application in desktop Chrome/Firefox/WebKit, with keyboard interaction and 390 CSS-pixel reflow. No physical-mobile, hearing, or human-preference claim.',
...
for (const [name, type] of [['Chrome', playwright.chromium], ['Firefox', playwright.firefox], ['WebKit', playwright.webkit]]) {
    let browser;const checks = [], errors = [], failures = [];
    try {
        browser = await type.launch({ headless: true, ...(name==='Chrome'?{executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'}:{}) });
        ...
        const surface=section.getByRole('group',{name:'Voicing surface'});
        assert(await surface.getByRole('radio',{name:'Recommended',exact:true}).isChecked());
        const recommendedIds=await cards.evaluateAll(elements=>elements.map(e=>e.dataset.voicingId));
        const recommendedCount=Number(await result.getAttribute('data-results-count'));
        const primary=await section.innerText();
        assert(!/wide-or-complex|product-extrapolated|source-observed|CAGED|Documented reference shape|recommendedRepresentative|Demand tier/i.test(primary));
        assert.equal(await section.locator('fieldset').filter({has:page.locator('legend').filter({hasText:/demand|tier|template/i})}).count(),0);
        checks.push('Recommended default and primary UI omit all internal policy/template controls');
        await surface.getByRole('radio',{name:'All voicings',exact:true}).check();
        await page.waitForFunction(n=>Number(document.querySelector('[data-results-count]')?.getAttribute('data-results-count'))>n,recommendedCount);
        assert.equal(await selected(),initial);checks.push('All expands full reachability without changing selection');
        await surface.getByRole('radio',{name:'Recommended',exact:true}).focus();await page.keyboard.press('Space');
        await page.waitForFunction(n=>Number(document.querySelector('[data-results-count]')?.getAttribute('data-results-count'))===n,recommendedCount);
        assert.deepEqual(await cards.evaluateAll(elements=>elements.map(e=>e.dataset.voicingId)),recommendedIds);
        checks.push('Keyboard surface switch restores identical Recommended first page and selected identity');
        ...
        assert(detailsText.includes('Selected by the recommendation policy.'));assert(detailsText.includes('separate Physical evidence'));
        for (const text of ['Human validation: absent.', 'Partial-cover groups:', '647.7 mm (default)', 'ordering preference only, independent of physical status', 'Base preference: classic-v1.', 'Recommended uses the separate recommendation policy.']) assert.ok(detailsText.includes(text), text);
        ...
        assert.equal(await selected(), initial);assert.ok((await section.innerText()).includes('outside this surface or these filters'));
        ...
        const screenshot = `docs/implementation/screenshots/product-${name.toLowerCase()}-390px.png`;
...
await writeFile('docs/implementation/product-release-runtime-ui.json', JSON.stringify(evidence, null, 2) + '\n');
