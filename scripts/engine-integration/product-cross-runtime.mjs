...
import { productConformance } from './scripts/engine-integration/product-conformance';
...
export function run(){
  ...
  return {versions:ENGINE_VERSIONS,cases,geometry,thresholds,ties,physical,crossing,product:productConformance()};
}`;
...
let priorEvidence;
try { priorEvidence = JSON.parse(await readFile('docs/implementation/product-cross-runtime.json', 'utf8')); } catch { /* First evidence run. */ }
...
try {
    ...
    await writeFile('docs/implementation/product-cross-runtime.json', JSON.stringify(evidence, null, 2) + '\n');
