...
// Default: every standard request once, with exact Recommended counts/first-six
// assertions against the complete offline policy census. Repeated latency gates
// are measured separately by product-ui-performance.mjs (480 requests x30).
...
const runs = Number(process.env.ENGINE_BENCH_RUNS ?? 1);
...
const policyBytes=await readFile('docs/implementation/product-policy-census.json');
...
const output = process.env.ENGINE_BENCH_OUTPUT ?? 'docs/implementation/product-ui-census.json';
...
async function save(complete) {
    ...
    const evidence = { schema: 'product-release-ui-census-v1', complete, startedAt, measuredAt: new Date().toISOString(),
        policyCensusSha256:createHash('sha256').update(policyBytes).digest('hex'),
        assertions:'Every measured request agrees with the complete census on Recommended surface, exact matching count and first-six allocation states, and with Stage 9 on structural/PASS/UNCERTAIN/REJECT counts.',
    ...
    for (const group of fixtureGroups) {
        ...
        for (let run = 0; run < runs; run++) {
            for (const root of group.roots) {
                ...
                const expectedPolicy=policy.rows.find(row=>row.chordId===group.chordId&&row.context===group.context&&row.root===root);
                assert.equal(sample.surface,'recommended');assert.equal(sample.matching,expectedPolicy.recommendedCount);
                assert.deepEqual(sample.firstStates,expectedPolicy.recommendedTop18.slice(0,6).map(row=>row.states),'Release UI first6 differs from complete policy census');
...
function installMeasurement() {
    ...
    window.Worker = new Proxy(window.Worker, { construct(Target, args) {
        ...
        worker.postMessage = (message, ...rest) => {
            if (message.protocol === 'engine-worker-v2' && ['START', 'PAGE', 'SET_VIEW'].includes(message.kind)) {
                measure.serial++; measure.start = { serial: measure.serial, at: performance.now(), kind: message.kind, sessionId: message.sessionId, operationId: message.operationId,
                    ...(message.kind === 'START' ? { intent: { chordId: message.payload.intent.chordId, rootPitchClass: message.payload.intent.rootPitchClass, context: message.payload.intent.context,surface:message.payload.view?.surface } } : {}) };
        ...
        worker.addEventListener('message', event => {
            ...
            if (message?.protocol !== 'engine-worker-v2') return;
            ...
            if (message.kind === 'EXACT_PAGE') {
                ...
                measure.exact = { serial: measure.start.serial, at: performance.now(), rows: message.payload.page.rows.length,
                    matching:s.matching,firstStates:message.payload.page.rows.map(row=>row.candidate.states),structural: s.structural, pass: s.pass, uncertain: s.uncertain, reject: s.reject, mode: s.mode, retainedBufferBytes: s.retainedBufferBytes, accountedBufferBytes: s.accountedBufferBytes };
            }
        }); return worker;
    } });
}
    ...
    window.engineBenchControls = {
        ...
        async measure(fixture) {
            ...
            const result = { firstPageMs: first.rendered - first.started, workerPostAfterActionMs: start.at - first.started,
                ...
                surface:start.intent.surface,matching:snapshot.matching,firstStates:snapshot.firstStates,reject: snapshot.reject, mode: snapshot.mode, retainedBufferBytes: snapshot.retainedBufferBytes, accountedBufferBytes: snapshot.accountedBufferBytes };
