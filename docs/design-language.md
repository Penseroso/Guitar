...
- Voice correspondence is horizontal: retain common-tone lanes; link actual notes; stack lanes by pitch, highest on top (destination in close root position, earlier chords following their lines), with one row per voice line and an empty slot where a line has no tone. Keep many-to-one resolutions visible. Do not independently re-sort every chord into formula order.
