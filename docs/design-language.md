...
- Comparison uses A/B structure and non-directional common/chromatic connections; a functional approach uses a directional path. Tritone original/substitute views stay in a stable A/B order with the same role rows. Comparison links are not a new theory rule or inferred performance.
- Root-motion labels use spelled interval direction. Pitch-class distances and shared/removed/added counts belong in Details. Hearing guide tones and selecting notes should highlight relevant connections, not trigger decorative animations. Line playback exists only for motion; comparisons offer Play relation alone.
