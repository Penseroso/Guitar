...
// Legacy chord-gallery SVG renderer preserved for the Stage 0 chord refactor.
export const ChordBox: React.FC<ChordBoxProps> = ({ fingering, labelMode, isMagnified }) => {
    ...
    const getFretY = (f: number) => topPadding + (f - minFret) * fretSpacing + (fretSpacing / 2);
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            >
                ...
                {Array.from({ length: 6 }).map((_, i) => {
                    const s = 5 - i;
                    const f = fingering.find(item => item.string === s);
