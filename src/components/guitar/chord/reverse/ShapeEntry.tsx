...
// component, no cut-down fret window: Fretboard's own onCellClick support (additive, opt-in) is
// what makes this interactive.
export const MAX_MODELED_FRET = 15;
...
export interface ShapeEntryProps {
     ...
     *  names"), notes are labeled/colored as intervals from THAT reading's real root instead of
     *  the plain bass-anchored default — see the note on bassPitchClass below for why this can't
     *  just be a blanket "show intervals" toggle. */
    focusedRootPitchClass?: number | null;
}
...
export function ShapeEntry({ states, onChange, onSelectFret, focusedRootPitchClass = null }: ShapeEntryProps) {
    ...
    // names which reading's root to use, via focusedRootPitchClass.
    ...
    return <div className={styles.neckScroll} tabIndex={-1} aria-label="Guitar shape entry — click a string/fret cell to place or remove a note">
        <Fretboard
            activeNotes={[]}
            rootNote={focusedRootPitchClass ?? bassPitchClass}
            showIntervals={focusedRootPitchClass !== null}
            chordTones={[]}
            modifierNotes={[]}
