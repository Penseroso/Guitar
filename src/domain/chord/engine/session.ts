...
export interface OrderKey extends RankKey {distance:number}
export interface PageCursor {
    schema:'cursor-v1';requestKey:string;profileKey:string;viewKey:string;
    versions:typeof ENGINE_VERSIONS;
    last:OrderKey&{allocationId:string};
}
export interface Counts {structural:number;pass:number;uncertain:number;reject:number;survivors:number;matching:number;matchingPass:number;matchingUncertain:number;assessed:number}
...
const emptyCounts=():Counts=>({structural:0,pass:0,uncertain:0,reject:0,survivors:0,matching:0,matchingPass:0,matchingUncertain:0,assessed:0});
export const compareOrder=(a:OrderKey,b:OrderKey)=>a.distance-b.distance||compareRank(a,b);
...
export class EngineSession {
    ...
    lookup(id:string):PresentationCandidate {
        ...
        return freeze({candidate,physical,facts,rank,displayRank:null,labels});
    }
    begin(viewInput:unknown={},pageSize=6,after:unknown=null,checkpoint?:ScanCheckpoint):PageScan {
        const view=createViewMatcher(this.request,viewInput);
    ...
    private validateCursor(input:unknown,view:ReturnType<typeof createViewMatcher>):PageCursor|null {
        if(input===null||input===undefined)return null;
        try {
            const c=record(input,['schema','requestKey','profileKey','viewKey','versions','last'],'cursor');
            if(c.schema!=='cursor-v1'||c.requestKey!==this.request.requestKey||c.profileKey!==this.request.physicalProfile.key
                ||c.viewKey!==view.key||canonical(c.versions)!==canonical(ENGINE_VERSIONS)) throw new Error('Incompatible keys.');
            const last=record(c.last,['allocationId','scoreNumerator','tie','distance'],'cursor.last');
...
export class PageScan {
    ...
    constructor(private session:EngineSession,private serial:number,readonly view:ReturnType<typeof createViewMatcher>,
    ...
    step(maxNodes=256):boolean {
        ...
        if(this.cached) {
            ...
            while(this.cacheIndex<end){const row=this.session.store.read(this.cacheIndex++);this.consume(row.states,row.status,row.scoreNumerator);}
            ...
            for(const states of batch.candidates) {
                ...
                this.consume(states,screen.status,score,matches);
            }
    ...
    private consume(states:Six<number>,status:'PASS'|'UNCERTAIN',score?:number,matches=this.view.matches(states,status)) {
        ...
        const row:HeapRow={tie:states,status,scoreNumerator:score??scoreFeatures(this.session.projector(states)),distance:this.view.distance(states)};
